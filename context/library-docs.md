...
- Task-comment replies and explicit member notifications reuse the `mention` category and deterministic `task-comment/{commentId}/{userId}` idempotency key. Delivery revalidates active membership, project access, task state, comment deletion state, and the comment audience; comment bodies are never copied into email or notification records.
...
- The shared UI foundation uses shadcn preset `b311momZs0` (`radix-maia`, `mist`) with the generated `shadcn/tailwind.css` utilities.
- Preset-regenerated primitives use the consolidated `radix-ui` package and `cn` package. Existing primitives not regenerated by the preset may retain focused `@radix-ui/*` imports; check compatibility before changing either pattern.
- `react-day-picker` is v10. Use `autoFocus`, not the removed `initialFocus` prop.
