...
**Synchronization note:** Timer stops publish an explicit cross-context event through BroadcastChannel with a storage-event fallback. The Picture-in-Picture document subscribes directly and closes on `timer-stopped`, even when its opener is minimized or throttled; the opener then reconciles against server-authoritative state.
