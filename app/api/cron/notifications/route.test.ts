...
vi.mock("server-only", () => ({}));
vi.mock("@/domain/notifications/scheduler", () => ({ runScheduledNotifications: mocks.run }));
...
const request = (authorization?: string) => new Request("https://example.com/api/cron/notifications", { method: "POST", headers: authorization ? { authorization } : {} });

describe("scheduled notifications cron route", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.secret.mockReturnValue(SECRET); mocks.run.mockResolvedValue({ runId: "run-1", counts: {}, cursor: null }); });
  ...
  it("fails closed when configuration is missing and permits scheduler retries on failure", async () => {
    mocks.secret.mockImplementationOnce(() => { throw new Error("missing"); });
