...
async function handleScheduledNotifications(request: Request) {
  let secret: string;
  try {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Scheduled notification job failed" },
      { status: 500 },
    );
  }
}

export const GET = handleScheduledNotifications;
export const POST = handleScheduledNotifications;
