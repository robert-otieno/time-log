...
export async function invitePersonAction(formData: FormData) {
  ...
  const client = role !== "client" ? { mode: "none" as const }
    : newClientName ? { mode: "create" as const, name: newClientName }
      : { mode: "existing" as const, clientId: existingClientId };
  let outcome = "failed";
  try {
    const result = await createInvitation(actor, organizationId, {
      email: String(formData.get("email") ?? ""), role, client,
      projectIds: formData.getAll("projectIds").map(String),
    }, createRequestCorrelation());
    outcome = "queued";
    try {
      const delivery = await deliverNotification(organizationId, result.notificationId);
      outcome = delivery.ok ? "sent" : "queued";
    } catch {
      // Provider configuration or availability cannot roll back the invitation.
    }
  } catch {
    // Validation and authorization errors return a safe generic result.
  }
  revalidatePath("/people");
  redirect(`/people?invite=${outcome}`);
}
...
export async function changeMembershipAccessAction(userId: string, action: "suspend" | "restore" | "remove") {
  ...
  try {
    ...
    return { ok: false as const, error: "We couldn’t update this person’s access." };
  }
}
