...
import { PROJECT_TOOLS, isProjectToolAvailable, projectToolFromPath } from "@/domain/projects/tools";
...
export default async function ProjectToolPage({ params }: { params: Promise<{ projectId: string; tool: string }> }) {
  ...
  const organizationId = await getActiveOrganizationId(actor); const access = await getAccessibleProject(actor, organizationId, projectId); if (!access || !isProjectToolAvailable(tool) || !access.project.enabledTools.includes(tool)) notFound();
