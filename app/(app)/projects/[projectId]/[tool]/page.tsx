...
export default async function ProjectToolPage({ params }: { params: Promise<{ projectId: string; tool: string }> }) {
  const actor = await getSessionActor(); if (!actor) redirect("/login?next=/projects"); const { projectId, tool: path } = await params; const tool = projectToolFromPath(path); if (!tool) notFound(); const access = await getAccessibleProject(actor, await getActiveOrganizationId(actor), projectId); if (!access || !access.project.enabledTools.includes(tool)) notFound(); const definition = PROJECT_TOOLS.find((item) => item.id === tool)!;
