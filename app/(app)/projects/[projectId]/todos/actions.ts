...
async function run<T>(projectId: string, operation: (value: Awaited<ReturnType<typeof context>>) => Promise<T>): Promise<TaskActionResult<T>> { try { const data = await operation(await context(projectId)); revalidatePath(`/projects/${projectId}/todos`); return { ok: true, data }; } catch (error) { return { ok: false, error: safeError(error) }; } }
