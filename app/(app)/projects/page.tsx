...
import { previewPendingLegacyTaskMigration } from "@/domain/tasks/migration";
...
export default async function ProjectsPage() {
  ...
  const migrationPreview = isClient
    ? null
    : await previewPendingLegacyTaskMigration(actor, organizationId);
    ...
    <main className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {isClient
            ? "Projects shared with your client account."
            : "Create workspaces and choose the tools each project needs."}
        </p>
      </div>
      <div
        className={
          membership.role === "admin"
            ? "grid gap-6 lg:grid-cols-[1fr_24rem]"
            : "grid gap-6"
        }
      >
          ...
          {active.length === 0 ? (
            <Card>
              <CardContent className="py-10 text-center text-muted-foreground">
                No current projects are available.
              </CardContent>
            </Card>
          ) : (
                    ...
                    <CardContent>
                      ...
                      {!isClient && (
                        <p className="mt-4 text-sm">
                          {project.enabledTools.length} enabled tools
                        </p>
                      )}
                    </CardContent>
                  </Card>
        ...
        {membership.role === "admin" && (
          <Card className="h-fit">
            <CardHeader>
              ...
              <CardDescription>
                Starts blank with To-dos and Time tracking.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ProjectForm clients={clients.map(toProjectFormClient)} />
            </CardContent>
          </Card>
        )}
      </div>
          ...
          projects={active
            .filter(
              (project) =>
                ...
                project.enabledTools.includes("todos"),
            )
