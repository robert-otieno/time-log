...
export async function readMessageBoard(actor: AuthActor, organizationId: string, projectId: string, includeArchived = false, before: string | null = null, db: Firestore = getAdminDb()) {
  ...
  return { posts: result, nextCursor, member: { userId: member!.userId, role: member!.role }, canCreatePosts: member!.role !== "client", canModerate: member!.role === "admin" || assignment?.projectRole === "admin" };
}
