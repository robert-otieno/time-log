...
export const auditFiltersSchema = z.object({ startDate: date, endDate: date, actorId: z.string().trim().max(128).optional(), action: z.enum(AUDIT_ACTIONS).optional(), outcome: z.enum(["succeeded", "denied", "failed"]).optional(), projectId: z.string().trim().max(128).optional(), targetType: z.enum(["session", "organization", "membership", "invitation", "client", "project", "task", "migration", "timer", "time-entry", "file", "message", "comment", "notification", "approval", "connector", "agent-run"]).optional(), cursor: z.string().max(512).optional() }).strict().refine((value) => value.endDate >= value.startDate && Date.parse(`${value.endDate}T00:00:00Z`) - Date.parse(`${value.startDate}T00:00:00Z`) <= 366 * 86_400_000, { message: "Choose a valid range of at most one year" });
