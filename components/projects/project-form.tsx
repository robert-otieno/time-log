...
export function ProjectForm({ clients, project }: { clients: ProjectFormClient[]; project?: ProjectFormProject }) {
  ...
  const [tools, setTools] = useState(project?.enabledTools ?? ["todos", "time"]);
  return <form action={action} className="space-y-5">
    ...
    <div className="space-y-2"><Label>Client company</Label><input type="hidden" name="clientId" value={clientId === "none" ? "" : clientId} /><Select value={clientId} onValueChange={setClientId}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No client</SelectItem>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select></div>
    <fieldset className="space-y-3"><legend className="text-sm font-medium">Enabled tools</legend><div className="grid gap-3 sm:grid-cols-2">{PROJECT_TOOLS.map((tool) => <label key={tool.id} className="flex items-center gap-2 rounded-md border p-3 text-sm"><Checkbox checked={tools.includes(tool.id)} onCheckedChange={(checked) => setTools((current) => checked ? [...current, tool.id] : current.filter((id) => id !== tool.id))} />{tool.label}</label>)}</div>{tools.map((tool) => <input key={tool} type="hidden" name="enabledTools" value={tool} />)}</fieldset>
