...
import { Clock3, Loader2, PictureInPicture2, Play, Plus, Square } from "lucide-react";
import { createTimerTaskAction, loadTimerLaunchOptionsAction, loadTimerStateAction, startTimerAction, stopTimerAction, type TimerLaunchOptions, type TimerView } from "@/app/(app)/timer-actions";
...
type PictureInPictureApi = { requestWindow(options?: { width?: number; height?: number; preferInitialWindowPlacement?: boolean }): Promise<Window>; window: Window | null };
...
function DetachedTimer({ timer, onStop }: { timer: TimerView; onStop(): Promise<string | null> }) {
  const [now, setNow] = useState(() => new Date(timer.observedAt).getTime()); const [pending, setPending] = useState(false); const [error, setError] = useState<string | null>(null);
  useEffect(() => { const interval = window.setInterval(() => setNow(Date.now()), 1000); return () => window.clearInterval(interval); }, []);
  const stop = async () => { setPending(true); setError(null); const message = await onStop(); setPending(false); if (message) setError(message); };
  return <main className="flex min-h-screen flex-col justify-between gap-4 bg-background p-4 text-foreground"><div><p className="text-xs text-muted-foreground">{timer.projectKey ? `${timer.projectKey} · ` : ""}{timer.projectName}</p><h1 className="mt-1 truncate text-base font-semibold">{timer.taskTitle ?? "Project-level work"}</h1>{timer.note && <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{timer.note}</p>}</div><div className="flex items-center justify-between gap-3"><Badge variant="secondary" className="font-mono text-base tabular-nums">{formatElapsedTimer(timer.startedAt, now)}</Badge><Button size="sm" variant="destructive" disabled={pending} onClick={() => void stop()}>{pending ? <Loader2 className="animate-spin" /> : <Square />}{pending ? "Stopping…" : "Stop & save"}</Button></div>{error && <p role="alert" className="text-xs text-destructive">{error}</p>}<p className="text-[11px] text-muted-foreground">Saves as internal, non-billable time. Use Time Log for other options.</p></main>;
}
...
export function GlobalTimerControl({ initialTimer, canTrack }: { initialTimer: TimerView | null; canTrack: boolean }) {
  ...
  const [reportingStatus, setReportingStatus] = useState<"internal" | "approved">("internal");
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      setPipSupported("documentPictureInPicture" in window && window.isSecureContext);
    });
  ...
  const openStop = () => { setStopNote(timer?.note ?? ""); setBillable(false); setReportingStatus("internal"); setError(null); setStopOpen(true); };
  const finishTimer = () => startStopping(async () => {
    setError(null);
    try {
      const response = await stopTimerAction({ note: stopNote.trim() || null, billable, clientReportingStatus: reportingStatus });
      if (!response.ok) { setError(response.code === "session_expired" ? "Your session expired. Sign in again to continue." : response.code === "time_duration_unreasonable" ? "This timer is too long to stop automatically. Contact an administrator to recover it." : "The timer could not be stopped. Try again."); return; }
      setTimer(null); setStopOpen(false); channelRef.current?.postMessage({ type: "timer-changed" }); router.refresh();
    } catch { setError("The timer could not be stopped. Check your connection and try again."); }
  });

  const stopFromDetachedWindow = async () => {
    try {
      const response = await stopTimerAction({ note: timer?.note ?? null, billable: false, clientReportingStatus: "internal" });
      if (!response.ok) {
        return response.code === "session_expired" ? "Your session expired. Sign in from Time Log to continue." : response.code === "time_duration_unreasonable" ? "This timer is too long to stop automatically. Contact an administrator." : "The timer could not be stopped. Try again.";
      }
      setTimer(null);
      channelRef.current?.postMessage({ type: "timer-changed" });
      router.refresh();
  ...
  const openPictureInPicture = async () => {
    try {
      ...
      const detached = await api.requestWindow({ width: 360, height: 190, preferInitialWindowPlacement: false });
      document.querySelectorAll('link[rel="stylesheet"], style').forEach((node) => detached.document.head.append(node.cloneNode(true)));
      detached.document.documentElement.lang = document.documentElement.lang;
      detached.document.documentElement.className = document.documentElement.className;
      detached.document.body.className = "bg-background text-foreground";
      detached.document.title = "Active timer · Time Log";
      detached.addEventListener("pagehide", () => setPipWindow(null), { once: true });
      setPipWindow(detached);
    } catch {
      toast.error("The always-on-top timer could not be opened. Check your browser's Picture-in-Picture permissions.");
    }
  };

  return <><aside className="fixed bottom-4 left-1/2 z-50 flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center justify-between gap-3 rounded-xl border bg-background/95 p-3 shadow-lg backdrop-blur" aria-label="Global timer">
    {timer ? <><RunningTimer timer={timer} /><div className="flex shrink-0 items-center gap-1">{pipSupported && <Button size="icon" variant="ghost" aria-label="Keep timer visible" title="Keep timer visible" onClick={() => void openPictureInPicture()}><PictureInPicture2 /></Button>}{syncIssue === "session" ? <Badge variant="destructive" asChild><Link href="/login">Sign in</Link></Badge> : syncIssue === "network" ? <Badge variant="outline">Offline</Badge> : <Button size="sm" variant="outline" onClick={openStop}><Square />Stop</Button>}</div></> : <><div className="flex items-center gap-2 text-sm"><Clock3 className="size-4 text-muted-foreground" /><span>{syncIssue === "network" ? "Timer status offline" : "No timer running"}</span></div>{syncIssue === "session" ? <Button asChild size="sm" variant="outline"><Link href="/login">Sign in</Link></Button> : <Button size="sm" onClick={() => void openLauncher()}><Play />Start timer</Button>}</>}
  </aside>
  ...
  <Dialog open={stopOpen} onOpenChange={(next) => !stopping && setStopOpen(next)}><DialogContent><DialogHeader><DialogTitle>Stop timer and save time?</DialogTitle><DialogDescription>The server will calculate the final duration and create a time entry for {timer?.taskTitle ?? "this project"}.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="stop-note">Work note</Label><Textarea id="stop-note" value={stopNote} onChange={(event) => setStopNote(event.target.value)} maxLength={2000} disabled={stopping} /></div><label className="flex items-center gap-2 text-sm"><Checkbox checked={billable} onCheckedChange={(checked) => setBillable(checked === true)} disabled={stopping} />Billable time</label><div className="space-y-2"><Label htmlFor="reporting-status">Client reporting</Label><Select value={reportingStatus} onValueChange={(value: "internal" | "approved") => setReportingStatus(value)} disabled={stopping}><SelectTrigger id="reporting-status" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="internal">Internal</SelectItem><SelectItem value="approved">Approved for client reporting</SelectItem></SelectContent></Select><p className="text-xs text-muted-foreground">Approval makes this entry eligible for future client-safe reports; it does not grant project access.</p></div></div>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}<DialogFooter><Button variant="outline" disabled={stopping} onClick={() => setStopOpen(false)}>Keep running</Button><Button disabled={stopping} onClick={finishTimer}>{stopping ? <Loader2 className="animate-spin" /> : <Square />}{stopping ? "Stopping…" : "Stop and save"}</Button></DialogFooter></DialogContent></Dialog>
  {pipWindow && timer && createPortal(<DetachedTimer timer={timer} onStop={stopFromDetachedWindow} />, pipWindow.document.body)}</>;
}
