...
import { FolderKanban, House, Menu, PanelsTopLeft, Settings, ShieldCheck } from "lucide-react";
...
export function AppHeader({ projects, role }: { projects: HeaderProject[]; role: "admin" | "member" | "client" }) {
  ...
  const items = [{ href: "/", label: "My Work", icon: House, visible: !isClient, active: pathname === "/" }, { href: "/projects", label: "Projects", icon: PanelsTopLeft, visible: true, active: pathname.startsWith("/projects") }, { href: "/settings", label: "Settings", icon: Settings, visible: true, active: pathname.startsWith("/settings") }, { href: "/admin", label: "Admin", icon: ShieldCheck, visible: role === "admin", active: pathname.startsWith("/admin") }].filter((item) => item.visible);
